...
export function generateBOQ(
  ...
  // ── 1. Site Preparation ──
  add('Site Preparation', 'SP', 'Topsoil Clearing & Benchmark Boundary Demarcation', 'Sq Ft', area.plotAreaSqFt, rate(r.sitePreparationPerSqFt, m.structural), 'Total Station Survey', 'incl. grid layout', 'Plot Area × Rate');
  add('Site Preparation', 'SP', 'Excavation – Column Footing Pits & Sump Trench (JCB)', 'Cu M', Math.max(10, approxConcreteCuM * 0.45), rate(r.earthworkPerCuM, m.structural), 'Mechanical Excavation', '180 kN/sqm SBC assumption', 'Volume × Earthwork Rate');
  add('Site Preparation', 'SP', 'Anti-Termite Pre-Construction Soil Chemical Treatment', 'Sq Ft', area.plotAreaSqFt, 18, 'Chlorpyrifos 20% EC', 'IS 6313 compliant chemical emulsion', 'Plot Area × Rate');
  add('Site Preparation', 'SP', 'Temporary Site Shed, Storage Godown & Electrical Setup', 'Units', 1, 75000, 'GI Sheet Enclosure', 'incl. 3-phase connection', 'Lump sum setup');
  ...
  add('Plinth', 'PL', 'Damp Proof Course (DPC) M20 with Waterproofing', 'Sq Ft', area.buildableAreaSqFt, rate(18, m.structural), 'Dr. Fixit Pidiproof LW+', '50mm thick waterproof DPC layer', 'Plinth Area × Rate');
  ...
  // ── 6. Plastering ──
  add('Plastering', 'PL', 'Internal Cement Plaster 12mm (Smooth Trowel Finish)', 'Sq Ft', qty.internalWallAreaSqFt + qty.ceilingAreaSqFt, rate(28, m.finishing), 'OPC 53 + P-Sand (1:4)', 'smooth sponge / steel trowel finish', 'Internal Wall Area × Rate');
  add('Plastering', 'PL', 'External Sand-Faced Cement Plaster 20mm (Double Coat)', 'Sq Ft', qty.exteriorPaintAreaSqFt, rate(36, m.finishing), 'OPC 53 + M-Sand (1:5)', 'rough cast base + sponge finish', 'External Wall Area × Rate');

  // ── 7. Roofing & Parapet ──
  add('Roofing', 'RF', 'Terrace Brick Bat Coba & Integral Waterproofing Treatment', 'Sq Ft', area.terraceSqFt, rate(85, m.finishing), 'Dr. Fixit + IWC', 'IS 3067 slope with round vatas', 'Terrace Area × Rate');
  add('Roofing', 'RF', 'Roof Cement Screed & 2% Water Drainage Slope', 'Sq Ft', area.terraceSqFt, rate(45, m.finishing), 'OPC 53 screed mortar', 'guarantees zero water ponding', 'Terrace Area × Rate');
  add('Roofing', 'RF', 'Parapet Wall RCC / Masonry 900mm Height with Plaster', 'RM', Math.round(buildingModel.externalPerimeterFt * 0.9), rate(2200, m.structural), 'M20 RCC + Plaster', '900mm fall protection barrier', 'Parapet Length × Rate');
  ...
  if (hasDoorsSelection && doorSchedule.length > 0) {
    ...
    add('Doors & Joinery', 'DJ', 'Solid Teak Wood Main & Hardwood Internal Door Frames', 'RM', (qty.mainDoorsCount + qty.internalDoorsCount) * 6.5, rate(1800, m.joinery), 'Plantation Teak 4×3 inch', 'seasoned kiln-dried hardwood', 'Frame Length × Rate');
    add('Doors & Joinery', 'DJ', 'SS Mortise Lock Sets, Handles, Hinges & Tower Bolts', 'Sets', qty.internalDoorsCount, rate(3200, m.joinery), 'Godrej / Yale SS 304', '6-lever mortise lock + SS hinges', 'Lock Sets Count × Rate');
  }
  ...
  if (hasPlumbingSelection) {
    ...
    add('Plumbing & Sanitary', 'PS', 'SWR Soil, Waste & Rainwater Drainage Pipe Lines', 'Metres', qty.swrDrainMetres, rate(r.swrPipePerMetre, m.mep), 'Supreme Ring-fit SWR', '110mm + 75mm dia lines', 'SWR Length × Rate');
    add('Plumbing & Sanitary', 'PS', 'Stainless Steel Floor Traps & Cockroach-Proof Gratings', 'Units', qty.floorTrapsCount, rate(850, m.mep), 'Jaquar / Cera SS Floor Trap', '50mm water seal depth', 'Traps Count × Rate');
    ...
    add('Plumbing & Sanitary', 'PS', `Overhead 4-Layer Anti-Bacterial Water Tank (${qty.overheadTankLitres}L)`, 'Units', 1, rate(28000, m.mep), 'Sintex 4-layer UV', 'with auto ball-valve & overflow line', 'Tank Units × Rate');
    add('Plumbing & Sanitary', 'PS', 'Constant Pressure Booster Pump & Sump Delivery System', 'Units', 1, rate(35000, m.mep), 'Grundfos CM / Wilo', '0.75HP VFD constant pressure pump', 'Pump Units × Rate');
    add('Plumbing & Sanitary', 'PS', 'Kitchen Double-Bowl SS 304 Sink with Swivel Mixer', 'Units', input.rooms.kitchen || 1, rate(14500, m.fixtures), 'Nirali / Franke 304', '304 grade anti-scratch with mixer tap', 'Sink Units × Rate');
  }
  ...
  if (hasPaintingSelection) {
    ...
    add('Painting & Waterproofing', 'PW', 'Interior Gypsum / Acrylic Wall Putty + Alkali Resistant Primer', 'Sq Ft', qty.puttyAreaSqFt, rate(16, m.finishing), 'Acrylic Putty + Primer', '2 coats putty + 1 coat primer', 'Putty Area × Rate');
    ...
    add('Painting & Waterproofing', 'PW', 'Bathroom Sunken Slab & Wall Upturn Elastomeric Waterproofing', 'Sq Ft', qty.bathroomWaterproofingSqFt, rate(r.waterproofingPerSqFt, m.finishing), 'Dr. Fixit Fastflex 2C', '1.0 ft vertical wall upturn membrane', 'Waterproofing Area × Rate');
  }
  ...
  if (hasElectricalSelection && (input.rooms.bedrooms || input.rooms.living || input.rooms.office)) {
    ...
    if (fanCount > 0) {
      add('Fixtures & Finishes', 'FF', 'Energy-Efficient BLDC 28W Ceiling Fans with Remote', 'Units', fanCount, rate(6500, m.fixtures), 'Atomberg Renesa BLDC', '28W brushless DC motor, 5-speed', 'Fans Count × Rate');
    }
  }
  if (hasFlooringSelection && (input.rooms.kitchen || 0) > 0) {
    add('Fixtures & Finishes', 'FF', 'Modular Kitchen Cabinets, Marine Ply Carcass & Counter', 'RM', (input.rooms.kitchen || 0) * 12, rate(18000, m.finishing), 'Sleek / Haecker Modular', 'Quartz countertop + BWP carcass', 'Counter Length × Rate');
  }
  if (input.liftRequired) {
    add('Fixtures & Finishes', 'FF', 'Residential 6-Passenger Machine-Room-Less (MRL) Lift Package', 'Units', 1, rate(580000, m.fixtures), 'Schindler / Otis 6-person', '630 kg payload with ARD safety device', 'Lift Package Allowance');
  }
  if (input.evCharging) {
    add('Fixtures & Finishes', 'FF', 'Fast AC EV Wallbox Charger (7.2 kW Type 2 Dedicated Point)', 'Units', input.carCount || 1, rate(38000, m.mep), 'Tata Power EZ Charge 7.2kW', 'Type 2 AC wallbox charger', 'EV Points × Rate');
  }
