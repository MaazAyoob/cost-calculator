...
export const LivePreviewPanel: React.FC = () => {
  ...
  return (
    <aside className="w-full bg-white rounded-2xl border border-[#E5E7EB] p-4 xl:p-5 shadow-xs flex flex-col justify-between space-y-3.5 text-left select-none">
      
      {/* ── 1. COMPACT HERO ESTIMATE HEADER ── */}
      <div className="space-y-2 pb-2.5 border-b border-[#E5E7EB]">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5">
            {costDelta !== null && costDelta !== 0 && (
              ...
              >
                {costDelta > 0 ? <TrendingUp className="w-2.5 h-2.5" /> : <TrendingDown className="w-2.5 h-2.5" />}
                <span>{costDelta > 0 ? `+${formatCurrency(costDelta)}` : formatCurrency(costDelta)}</span>
              </motion.span>
            )}

            <span className="text-[9px] font-mono font-bold text-[#4B5563] bg-[#F8F8F6] px-1.5 py-0.5 rounded border border-[#E5E7EB]">
              LIVE SYNC
            </span>
          </div>
        </div>
        ...
        <div className="flex items-baseline gap-2 flex-wrap">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="text-2xl sm:text-3xl font-black text-[#1B3D34] tracking-tight font-heading leading-none"
            >
        ...
        {/* Clean Integrated Architectural Stats Line */}
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-[#4B5563]">
          <button
            type="button"
            onClick={() =>
              openInspector(
                'Built-Up Area (BUA)',
                'Total BUA = Footprint per Floor × Storeys',
                [
                  { label: 'Footprint', value: `${area.buaPerFloorSqFt || 0} sq.ft` },
                  { label: 'Storeys', value: `${floors || 1} Floors` },
                ],
                'BBMP / BDA Municipal Zoning Regulations',
                `${buaSqFt.toLocaleString()} sq.ft`
              )
            }
            className="flex items-center gap-1 hover:text-[#1B3D34] cursor-pointer"
          >
            <span className="font-bold text-[#1B3D34] font-mono">{buaSqFt > 0 ? `${buaSqFt.toLocaleString()} sq.ft` : '0 sq.ft'}</span>
            <span>BUA</span>
            <Info className="w-2.5 h-2.5 opacity-50 hover:opacity-100" />
          </button>
          ...
          <button
            ...
            onClick={() =>
              openInspector(
                ...
                'Remaining Ground = Total Plot Area - Ground Built-up Footprint',
                [
                  { label: 'Plot Area', value: `${plotArea.toLocaleString()} sq.ft` },
                  { label: 'Ground Footprint', value: `${area.buaPerFloorSqFt || 0} sq.ft` },
                ],
                'Open garden & setback yard space',
                `${Math.round(remainingGround).toLocaleString()} sq.ft`
              )
            }
            className="flex items-center gap-1 hover:text-[#1B3D34] cursor-pointer"
          >
            <span className="font-bold text-[#1B3D34] font-mono">{remainingGround > 0 ? `${Math.round(remainingGround).toLocaleString()} sq.ft` : '0 sq.ft'}</span>
            <span>Open Yard</span>
            <Info className="w-2.5 h-2.5 opacity-50 hover:opacity-100" />
          </button>
          ...
          <span className="font-mono text-[#4B5563]">
            {plotArea > 0 ? `${plotArea.toLocaleString()} sq.ft site` : '0 sq.ft'}
          </span>
        </div>

      </div>

      {/* ── 2. PROMINENT TALLER 3D ARCHITECTURAL CANVAS ── */}
      <div className="w-full h-60 sm:h-64 lg:h-72 xl:h-80 rounded-xl overflow-hidden border border-[#E5E7EB] relative bg-[#1B3D34] shrink-0 shadow-2xs">
      ...
      {/* ── 3. COMPACT BOTTOM INTELLIGENCE & HEAD ALLOCATION ── */}
      <div className="pt-2 border-t border-[#E5E7EB] space-y-2">
        
        {/* Step-specific concise takeoff */}
        {currentStep === 1 && (
          <div className="flex items-center justify-between text-[11px] p-2 bg-[#F8F8F6] rounded-lg border border-[#E5E7EB]">
            <div>
              <span className="text-[#4B5563] block text-[10px]">Setbacks (Front • Rear • Sides)</span>
              <span className="font-bold text-[#1B3D34] font-mono">
                {area.setbacks?.frontSetbackFt ?? 3.5}' • {area.setbacks?.rearSetbackFt ?? 3.0}' • {area.setbacks?.leftSetbackFt ?? 3.0}'
              </span>
            </div>
            <div className="text-right">
              <span className="text-[#4B5563] block text-[10px]">Buildable Footprint</span>
              <span className="font-bold text-[#1B3D34] font-mono">
        ...
        {currentStep === 2 && (
          <div className="grid grid-cols-4 gap-1.5 text-center text-[11px]">
            <div className="p-1.5 bg-[#F8F8F6] rounded-lg border border-[#E5E7EB]">
              <span className="text-[9px] font-bold text-[#4B5563] block">Doors</span>
              <span className="font-bold text-[#1B3D34] font-mono">{quantities.totalDoorsCount || 8}</span>
            </div>
            <div className="p-1.5 bg-[#F8F8F6] rounded-lg border border-[#E5E7EB]">
              <span className="text-[9px] font-bold text-[#4B5563] block">Windows</span>
              <span className="font-bold text-[#1B3D34] font-mono">{quantities.windowsCount || 10}</span>
            </div>
            <div className="p-1.5 bg-[#F8F8F6] rounded-lg border border-[#E5E7EB]">
              <span className="text-[9px] font-bold text-[#4B5563] block">Switch Pts</span>
              <span className="font-bold text-[#1B3D34] font-mono">{quantities.totalElectricalPoints || 120}</span>
            </div>
            <div className="p-1.5 bg-[#F8F8F6] rounded-lg border border-[#E5E7EB]">
              <span className="text-[9px] font-bold text-[#4B5563] block">Paint sq.ft</span>
        ...
        {currentStep === 3 && (
          <div className="grid grid-cols-2 gap-2 text-[11px]">
            <div className="p-2 bg-[#F8F8F6] rounded-lg border border-[#E5E7EB] flex justify-between items-center">
              <div>
                <span className="font-bold text-[#1B3D34] block">Rebar Steel</span>
                <span className="text-[9px] text-[#4B5563]">{materialBrands?.steel || 'Tata Tiscon'}</span>
              </div>
              <span className="font-bold text-[#1B3D34] font-mono">{quantities.steelTonnes || 0} T</span>
            </div>
            <div className="p-2 bg-[#F8F8F6] rounded-lg border border-[#E5E7EB] flex justify-between items-center">
              <div>
                <span className="font-bold text-[#1B3D34] block">Cement</span>
                <span className="text-[9px] text-[#4B5563]">{materialBrands?.cement || 'UltraTech'}</span>
              </div>
              <span className="font-bold text-[#1B3D34] font-mono">{(quantities.cementBags || 0).toLocaleString()} Bags</span>
            </div>
          </div>
        )}

        {currentStep >= 4 && currentStep <= 10 && (
          <div className="p-2 bg-[#F8F8F6] rounded-lg border border-[#E5E7EB] flex justify-between items-center text-[11px]">
            <div>
              ...
              <span className="text-[9px] text-[#4B5563]">Physical Takeoff Computed</span>
            </div>
            <span className="font-bold text-[#1B3D34] font-mono">
        ...
        {/* Compact 4-Head Allocation Line (Civil | Finishes | MEP | GST) */}
        {totalCost > 0 && (
          <div className="grid grid-cols-4 gap-1.5 pt-1 text-[10px] text-[#4B5563]">
            <div className="border-r border-[#E5E7EB] pr-1">
              <span className="block text-[8px] font-bold text-[#4B5563] uppercase">Civil</span>
              <span className="font-bold text-[#1B3D34] font-mono">{formatCurrency(structureCost)}</span>
            </div>
            <div className="border-r border-[#E5E7EB] pr-1">
              <span className="block text-[8px] font-bold text-[#4B5563] uppercase">Finishes</span>
              <span className="font-bold text-[#1B3D34] font-mono">{formatCurrency(finishesCost)}</span>
            </div>
            <div className="border-r border-[#E5E7EB] pr-1">
              <span className="block text-[8px] font-bold text-[#4B5563] uppercase">MEP</span>
              <span className="font-bold text-[#1B3D34] font-mono">{formatCurrency(mepCost)}</span>
            </div>
            <div>
              <span className="block text-[8px] font-bold text-[#4B5563] uppercase">GST (18%)</span>
