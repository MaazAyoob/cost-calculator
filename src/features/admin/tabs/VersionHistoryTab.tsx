...
export const VersionHistoryTab: React.FC = () => {
  ...
  const [selectedVersionForRollback, setSelectedVersionForRollback] = useState<string | number | null>(null);
              ...
              >
                <CardContent className="p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-bold font-mono text-slate-900">
                        {v.versionLabel || v.versionNumber || (v.id ? `ver-${String(v.id).slice(0, 8)}` : 'v1.0')}
                      </span>
                      {isActive ? (
                    ...
                    <p className="text-xs text-slate-600">
                      {v.changeSummary || v.changeNote || v.description || 'Approved production baseline configuration'}
                    </p>
                    <div className="flex items-center gap-3 text-[11px] text-slate-400 font-mono pt-1">
                      <span className="flex items-center gap-1">
                        <User className="w-3 h-3" />
                        {v.publishedBy || v.createdBy || 'System Architect'}
                      </span>
                      <span className="flex items-center gap-1">
                        <Calendar className="w-3 h-3" />
                        {v.publishedAt ? new Date(v.publishedAt).toLocaleDateString() : 'Baseline'}
                      </span>
                      <span>{v.parameterCount || v.parametersCount || 68} parameters snapshot</span>
                    </div>
                  </div>
                  ...
                  {!isActive && (
                    <Button
                      type="button"
                      onClick={() => setSelectedVersionForRollback(v.id || v.versionNumber)}
